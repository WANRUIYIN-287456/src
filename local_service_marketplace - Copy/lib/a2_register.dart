...
class _RegistrationScreenState extends State<RegistrationScreen> {
  ...
  final TextEditingController _passEditingController = TextEditingController();
  final TextEditingController _pass2EditingController = TextEditingController();
  final TextEditingController _otpEditingController = TextEditingController(); // Add this controller for OTP field
  bool _isChecked = false;
  bool _isTap = false;
  bool _isResendEnabled = false; // Track if resend button is enabled
  final _formKey = GlobalKey<FormState>();
  late double screenHeight, screenWidth, cardwidth;
  String eula = "";
  late String _imageString;
  String _otp = ''; // Store the generated OTP
          ...
          Padding(
            ...
            child: Card(
              ...
              child: Container(
                  ...
                  child: Column(
                    children: [
                      const Text("OTP Verification"), // Change title
                      Form(
                          key: _formKey,
                          child: Column(
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  Expanded(
                                    child: ElevatedButton(
                                        onPressed: onOTPDialog,
                                        child: const Text("Send OTP")), // Change button text
                                  ),
                                  const SizedBox(width: 16,),
                                  ElevatedButton(
                                      onPressed: _isResendEnabled ? onResendOTP : null, // Disable resend button initially
                                      child: const Text("Resend OTP")), // Resend OTP button
                                  const SizedBox(width: 16,),
                                  Expanded(
                                    child: ElevatedButton(
                                        onPressed: onRegisterDialog,
                                        child: const Text("OK")), // Change button text
                                  ),
                                ],
                              ),
                            ],
  ...
  void onRegisterDialog() {
    ...
    if (enteredOTP != _otp) {
      ...
      _otpEditingController.clear(); // Clear OTP field
      return;
    }
  ...
  void sendOTP() {
    _otp = generateOTP(); // Generate OTP
    // Send OTP via email
    // You'll need to implement PHP code for sending email with OTP
    // Make a POST request to your PHP endpoint passing email and OTP
    // The PHP code will handle sending email using PHP mailer
    // Example: http.post(Uri.parse("YOUR_PHP_ENDPOINT"), body: {"email": _emailEditingController.text, "otp": _otp});
    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
