...
if (isset($_POST['barteruserid']) && isset($_POST['orderbill'])){
	$barteruserid = $_POST['barteruserid'];	
	$orderbill = $_POST['orderbill'];
	$userid = $_POST['userid'];
	$sqlorderdetails = "SELECT * FROM `tbl_orderdetails` INNER JOIN `tbl_products` ON tbl_orderdetails.product_id = tbl_products.product_id WHERE tbl_orderdetails.barteruser_id = '$barteruserid' AND tbl_orderdetails.order_bill = '$orderbill' AND tbl_orderdetails.user_id = '$userid'";
}else{
    $sqlorderdetails = "NA";
}
...
if ($result->num_rows > 0) {
	...
	while ($row = $result->fetch_assoc()) {
        ...
        $orderdetailslist['product_id'] = $row['product_id'];
        $orderdetailslist['product_name'] = $row['product_name'];
        $orderdetailslist['orderdetail_qty'] = $row['orderdetail_qty'];
        $orderdetailslist['orderdetail_paid'] = $row['orderdetail_paid'];
        $orderdetailslist['barteruser_id'] = $row['barteruser_id'];
